...
TINY.table=function(){
	...
	sorter.prototype.init=function(){
		var initialization = 'yes';
		this.set(initialization); var t=this.t, i=d=0; t.h=T$$('tr',t)[0];
		t.l=t.r.length; t.w=t.r[0].cells.length; t.a=[]; t.c=[]; this.p.is=this.p.size;
		if(this.p.colddid){
			document.getElementById("columns").innerHTML=null;
			d=T$(this.p.colddid);
			var o=document.createElement('option'); o.value=-1; o.innerHTML='Tous'; d.appendChild(o)
		}
		for(i;i<t.w;i++){
			...
			if(d){
				alert
				if(this.p.Title!=undefined){
					if(T$$('h3',c)[0].title != ""){
						var o=document.createElement('option'); o.value=i; o.innerHTML=T$$('h3',c)[0].title; d.appendChild(o)
					}
				}else{
					if(T$$('h3',c)[0].innerHTML != ""){
		...
		this.reset(initialization)
	};
	sorter.prototype.reset=function(initialization){
	...
	sorter.prototype.sort=function(x,f,z){
		...
		for(i;i<t.l;i++){
			...
			if(v.hasChildNodes() == undefined){
				v.hasChildNodes() = true;
				alert(hasChildNodes);
			}
			while(v.hasChildNodes()){v=v.firstChild}
			t.a[i].v=v.nodeValue?v.nodeValue:''
		}
		if(!z &&!f){var initialisation = 'no';} else {var initialisation = 'yes';}
		for(i=0;i<t.w;i++){var c=t.h.cells[i]; if(c.getAttribute('figed') == "true"){ var nbcol = i; var figed = true; }if(c.className!='nosort'){c.className=this.p.headclass}}
		...
		for(i=0;i<t.l;i++){if(figed == true && nbcol != undefined){ var line= i+1; t.r[t.a[i].o].cells[nbcol].innerText = line; } var r=t.r[t.a[i].o].cloneNode(true); n.appendChild(r)}
		t.replaceChild(n,t.b); this.set(initialisation); this.alt(); if(this.p.paginate){this.size(z)} this.sethover();
	...
	sorter.prototype.set=function(initialization){
	...
	function decimals(n,d){alert(Math.round(n*Math.pow(10,d))/Math.pow(10,d)); return Math.round(n*Math.pow(10,d))/Math.pow(10,d)};
	function cp(f,c){
		var g,h,tmp,f2,c2; f=g=f.v.toLowerCase(), c=h=c.v.toLowerCase(), reg = /[0-9]{1,4}\W[0-9]{1,2}\W[0-9]{1,4}/i;
		...
		//alert(i+','+n)
		if(!isNaN(i)&&!isNaN(n)){
			var i=f.replace(/\s?(R\$|\$|\%|\:|\,|\.|€)\s?/g,''), n=parseFloat(c.replace(/\s?(R\$|\$|\%|\:|\,|\.|€)\s?/g,''));
			if(parseFloat(i)==parseInt(i)){
				g=i,h=n
				//alert(g+','+h)
			}
			else{
		...
		c2[0] = c2[1];
		c2[1] = tmp;
		if(f2[3] != undefined && f2[4] != undefined){
			f2 = f2[0]+'/'+f2[1]+'/'+f2[2]+' '+f2[3]+':'+f2[4];
			c2 = c2[0]+'/'+c2[1]+'/'+c2[2]+' '+c2[3]+':'+c2[4];
		}
		else{
			f2 = f2.join('/');
			c2 = c2.join('/');
