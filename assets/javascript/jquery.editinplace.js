...
$.fn.editInPlace.defaults = {
	...
	save_button:		'<button class="inplace_save">Sauvegarder</button>', // string: image button tag to use as â€œSaveâ€� button
	cancel_button:		'<button class="inplace_cancel">Annuler</button>', // string: image button tag to use as â€œCancelâ€� button
	...
	default_text:		"X", // string: text to show up if the element that has this functionality is empty
...
$.extend(InlineEditor.prototype, {
	...
	createEditorElement: function() {
		if (-1 === $.inArray(this.settings.field_type, ['text', 'textarea', 'select','calendar','calendar_2','hour']))
			throw "Unknown field_type <fnord>, supported are 'text', 'textarea', 'calendar', 'hour' and 'select'";
		tmp = new Date();
		var Month = tmp.getMonth()+1;
		Date_2 = tmp.getDate()+'/'+Month+'/'+tmp.getFullYear()
		var editor = null;
		if ("select" === this.settings.field_type)
			editor = this.createSelectEditor();
		else if ("calendar" === this.settings.field_type)
			editor = $('<input type="text" onFocus="ds_sh(this);" readonly="readonly" style="cursor: text" ' + this.inputNameAndClass() 
				+ ' size="' + this.settings.text_size  + '" />');
		else if ("calendar_2" === this.settings.field_type)
			editor = $('<input type="text" onFocus="ds_sh(this,\''+Date_2+'\',\'2\');" readonly="readonly" style="cursor: text" ' + this.inputNameAndClass() 
				+ ' size="' + this.settings.text_size  + '" />');
		else if ("hour" === this.settings.field_type)
			editor = $('<input type="text" id="hours_timePicker" name="hours_timePicker" ' + this.inputNameAndClass() 
				+ ' size="' + this.settings.text_size  + '" />');
