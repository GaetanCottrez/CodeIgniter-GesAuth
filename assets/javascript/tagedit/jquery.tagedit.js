	...
	$.fn.tagedit = function(options) {
		...
		*/
		options = $.extend(true, {
			...
			texts: {
				removeLinkTitle: 'Retirer de la liste',
				saveEditLinkTitle: 'Sauvegarder les changements',
				deleteLinkTitle: 'Supprimer ce tag de la base de données',
				deleteConfirmation: 'Etes-vous sûr de vouloir supprimer cette entrée?',
				deletedElementTitle: 'Cet élément va être supprimé',
				breakEditLinkTitle: 'Annuler',
                forceDeleteConfirmation: 'Il y a plusieurs enregistrement qui utilisent ce tag, êtes-vous sûr de vouloir le supprimer?'
			}
		}, options || {});
		...
		if(document.getElementById('eventsapplied') != undefined){
			var eventsapplied = document.getElementById('eventsapplied').value;
		}else{
			var eventsapplied = 'onkeyup="this.value = checkSerialNumber(this.value.toUpperCase());"';
		}
		...
		*/
		function inputsToList() {
			...
			html += '<input type="text" name="'+baseName+'[]" value="" id="tagedit-input" disabled="disabled" class="tagedit-input-disabled" dir="'+options.direction+'" '+eventsapplied+' />';
			html += '</li>';
			html += '</ul>';
