...
{	
	function _compile_session()
	{
		...
		$output .= '<legend style="color:#900;">&nbsp;&nbsp;'.$this->CI->lang->line('profiler_sessions').'&nbsp;&nbsp;</legend>';
		$output .= "\n";
	...
	function _compile_version_files()
	{
		$output  = "\n\n";
		$output .= '<fieldset style="border:1px solid #009999;padding:6px 10px 10px 10px;margin:20px 0 20px 0;background-color:#eee">';
		$output .= "\n";
	...
	function _compile_memory_usage()
	{
		...
		{
			$output .= "<div style='color:#5a0099;font-weight:normal;padding:4px 0 4px 0'>".formatBytes($usage).' Mo</div>';
		}
		else
		{
	...
	function run()
	{
		...
		$output .= $this->_compile_version_files();
		$output .= '</div>';
